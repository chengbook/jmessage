...
import static com.jiguang.jmessageflutter.JsonUtils.toListJson;

/**
 * JmessageFlutterPlugin
 */
public class JmessageFlutterPlugin implements MethodCallHandler, FlutterPlugin, PluginRegistry.NewIntentListener, ActivityAware {


    private static final String TAG = "| JMessage | Android | ";
...
//  static final String ERR_MSG_PERMISSION_WRITE_EXTERNAL_STORAGE = "Do not have 'WRITE_EXTERNAL_STORAGE' permission";
    ...
    private void onAttachedToEngine(Context context, BinaryMessenger binaryMessenger) {
        ...
        this.channel = new MethodChannel(binaryMessenger, "jmessage_flutter");
    ...
    public void onMethodCall(MethodCall call, @NotNull Result result) {
        switch (call.method) {
            ...
            case "removeFromFriendList":
                ...
                break;
            case "updateFriendNoteName":
                updateFriendNoteName(call, result);
                break;
            case "updateFriendNoteText":
            ...
            case "setNoDisturb":
                ...
                break;
            case "getNoDisturbList":
                getNoDisturbList(result);
                break;
            case "setNoDisturbGlobal":
                setNoDisturbGlobal(call, result);
                break;
            case "isNoDisturbGlobal":
                isNoDisturbGlobal(result);
                break;
            case "blockGroupMessage":
                blockGroupMessage(call, result);
                break;
            case "isGroupBlocked":
                isGroupBlocked(call, result);
                break;
            case "getBlockedGroupList":
                getBlockedGroupList(result);
                break;
            case "downloadThumbUserAvatar":
                downloadThumbUserAvatar(call, result);
                break;
            case "downloadOriginalUserAvatar":
                downloadOriginalUserAvatar(call, result);
                break;
            case "downloadThumbImage":
            ...
            case "exitChatRoom":
                ...
                break;
            case "getChatRoomConversation":
                getChatRoomConversation(call, result);
                break;
            case "getChatRoomConversationList":
                getChatRoomConversationList( result);
                break;
            case "getAllUnreadCount":
                getAllUnreadCount(result);
                break;
            case "addGroupAdmins":
    ...
    private void setup(MethodCall call, Result result) {
        ...
        try {
            ...
            boolean r = JMessageClient.init(mContext, isOpenMessageRoaming);
    ...
    private void login(MethodCall call, final Result result) {
        ...
        JMessageClient.login(username, password, new BasicCallback() {
            @Override
            public void gotResult(int status, String desc) {
                if (status == 0 && mainActivity != null && mainActivity.getIntent() != null) {
                    sendNotificationTap(mainActivity.getIntent());
                }
                handleResult(status, desc, result);
            }
        });
    ...
    private void setBadge(Result result) {
        //  NOTE:  android do not support this function.
        result.success(null);
    }

    private void getMyInfo(Result result) {
        ...
        try {
            ...
            if (params.has("extras")) {
                ...
                while (it.hasNext()) {
                    ...
                    myInfo.setUserExtras(pair.getKey(), pair.getValue());
    ...
    private void sendDraftMessage(MethodCall call, final Result result) {
        ...
        try {
            ...
            message.setOnSendCompleteCallback(new BasicCallback() {
                ...
                public void gotResult(int status, String desc) {
                    if (status == 0) {
                        Map<String, Object> json = JsonUtils.toJson(message);
...
//                        String serverMsgID = details.getServerMsgID() + "";
    ...
    private void getHistoryMessages(MethodCall call, Result result) {
        ...
        try {
            ...
            if (params.has("isDescend")) {
                isDescend = params.getBoolean("isDescend");
            } else {
                isDescend = false;
            }
        ...
        ArrayList<Object> messageJSONArr = new ArrayList<>();
...
//    TODO: test JSONArray to dart.
    ...
    private void getFriends(final Result result) {
        ContactManager.getFriendList(new GetUserInfoListCallback() {

            @Override
            public void gotResult(int status, String desc, List list) {
                if (status == 0) {
                    handleResult(toListJson(Collections.singletonList(list)), status, desc, result);
    ...
    private void getGroupIds(final Result result) {
        JMessageClient.getGroupIDList(new GetGroupIDListCallback() {
            @Override
            public void gotResult(int status, String desc, List<Long> list) {
                if (status == 0) {
                    ArrayList<String> groupIdJsonArr = new ArrayList<>();
                    for (Long id : list) {
                        groupIdJsonArr.add(String.valueOf(id));
                    }
                    handleResult(Collections.singletonList(groupIdJsonArr), status, desc, result);
    ...
    private void getGroupMembers(MethodCall call, final Result result) {
        ...
        JMessageClient.getGroupMembers(groupId, new RequestCallback<List<GroupMemberInfo>>() {
            ...
            public void gotResult(int status, String desc, List<GroupMemberInfo> groupMemberInfos) {
                if (status == 0) {
                    handleResult(toListJson(Collections.singletonList(groupMemberInfos)), status, desc, result);
    ...
    private void getNoDisturbList(final Result result) {
        JMessageClient.getNoDisturblist(new GetNoDisurbListCallback() {
            ...
            public void gotResult(int status, String desc, List userInfoList, List groupInfoList) {
                if (status == 0) {
                    Map<String, Object> res = new HashMap<>();
                    res.put("userInfoArray", toListJson(Collections.singletonList(userInfoList)));
                    res.put("groupInfoArray", toListJson(Collections.singletonList(groupInfoList)));
    ...
    private void isNoDisturbGlobal(final Result result) {
    ...
    private void getBlockedGroupList(final Result result) {
        JMessageClient.getBlockedGroupsList(new GetGroupInfoListCallback() {
            ...
            public void gotResult(int status, String desc, List<GroupInfo> list) {
                ...
                handleResult(Collections.singletonList(res), status, desc, result);
            }
        });
    }
    ...
    private void exitConversation( Result result) {
    ...
    private void getConversations(Result result) {
        ...
        JMessageClient.getGroupInfo(groupId, new GetGroupInfoCallback() {
            ...
            public void gotResult(int status, String desc, GroupInfo groupInfo) {
                if (status == 0) {
                    ...
                    handleResult(toListJson(Collections.singletonList(groupSilenceMemberInfos)), status, desc, result);
                } else {
                    handleResult(status, desc, result);
                }
            }
...
//    TODO:
    ...
    private void getChatRoomConversationList( Result result) {
        ChatRoomHandler.getChatRoomConversationList( result);
    }

    private void getAllUnreadCount(Result result) {
...
//    TODO: test it.
    ...
    private void getPublicGroupInfos(MethodCall call, final Result result) {
        ...
        JMessageClient.getPublicGroupListByApp(appKey, start, count, new RequestCallback<List<GroupBasicInfo>>() {
            ...
            public void gotResult(int status, String desc, List<GroupBasicInfo> groupBasicInfos) {
                handleResult(toListJson(Collections.singletonList(groupBasicInfos)), status, desc, result);
            }
        });
    }
    ...
    public void onEventMainThread(MessageEvent event) {
        ...
        Map<String, Object> msgJson = toJson(event.getMessage());

        channel.invokeMethod("onReceiveMessage", msgJson);
    }


    @Override
    public boolean onNewIntent(Intent intent) {
        boolean result = sendNotificationTap(mainActivity.getIntent());
    ...
    private Boolean sendNotificationTap(Intent intent) {
        if (intent.getAction().equals("launcher_app_" + mContext.getPackageName())) {
            String json = intent.getStringExtra("msgJson");
            Gson gson = new Gson();
            HashMap map = gson.fromJson(json, HashMap.class);
            channel.invokeMethod("onClickMessageNotification", map);
            return true;
        }
        return false;
    ...
    public void onEventMainThread(NotificationClickEvent event) {
        ...
        launchIntent.setAction("launcher_app_" + mContext.getPackageName());
        launchIntent.addCategory(Intent.CATEGORY_LAUNCHER);
        launchIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_SINGLE_TOP);
        Map<String, Object> msgJson = toJson(event.getMessage());
        launchIntent.putExtra("msgJson", Json.encodeMap(msgJson));
        mContext.startActivity(launchIntent);

        channel.invokeMethod("onClickMessageNotification", msgJson);
    }


    /**
     * 同步离线消息。
     * TODO：cheng 2021年06月05日18:37:22 此处处理的有点复杂了，同步离线消息是不需要同时下载对应的多媒体文件；
     * TODO: 此处应该抛到上层处理；
     * TODO：应该保证同步离线消息的回调只会回传给flutter一次即可
     *
     * @param event 离线消息事件。
     */
    public void onEventMainThread(OfflineMessageEvent event) {
        ...
        final ArrayList<Object> msgJsonArr = new ArrayList<>();
        ...
        if (latestMediaMessageIndex == -1) { // 没有多媒体消息
            ...
            channel.invokeMethod("onSyncOfflineMessage", json);
        } else {
    ...
    private List<Map<String,Object>> mRoamingMessageCache;
...
//    TODO:
    ...
    void addSyncRoamingMessageListener(JSONArray data, Readable callback) {
        ...
        if (mRoamingMessageCache != null) { // 触发缓存
            for (Map<String,Object> json : mRoamingMessageCache) {
                channel.invokeMethod("onSyncRoamingMessage", json);
            }
    ...
    public void onEventMainThread(LoginStateChangeEvent event){
        ...
        channel.invokeMethod("onLoginStateChanged", json);
    }
    ...
    public void onEventMainThread(ContactNotifyEvent event) {
        ...
        channel.invokeMethod("onContactNotify", json);
    }
    ...
    public void onEventMainThread(MessageRetractEvent event)  {
        ...
        channel.invokeMethod("onRetractMessage", json);
    }
                ...
                event.getTargetInfo(new CommandNotificationEvent.GetTargetInfoCallback() {
                    ...
                    public void gotResult(int status, String desc, Object obj, CommandNotificationEvent.Type type) {
                        if (status == 0) {
                            ...
                            channel.invokeMethod("onReceiveTransCommand", result);
                        }
                    }
                });
            }
    ...
    public void onEventMainThread(ChatRoomMessageEvent event) {
        ...
        channel.invokeMethod("onReceiveChatRoomMessage", jsonArr);
    }
        ...
        event.getFromUserInfo(new GetUserInfoCallback() {
            ...
            public void gotResult(int status, String desc, UserInfo userInfo) {
                if (status == 0) {
                    ...
                    json.put("sendApplyUser", new HashMap<String, Object>());
                }
                ...
                event.getApprovalUserInfoList(new GetUserInfoListCallback() {
                    ...
                    public void gotResult(int status, String s, List<UserInfo> list) {
                        if (status == 0) {
                            json.put("joinGroupUsers", toListJson(Collections.singletonList(list)));
                        } else {
                            json.put("joinGroupUsers", new HashMap<String, Object>());
                        }
                        // 回调都回来了再发
                        channel.invokeMethod("onReceiveApplyJoinGroupApproval", json);
                    }
                });
            }
        });
    ...
    public void onEventMainThread(GroupApprovedNotificationEvent event) {
        ...
        event.getApprovedUserInfoList(new GetUserInfoListCallback() {
            ...
            public void gotResult(int status, String s, List<UserInfo> list) {
                if (status == 0) {
                    json.put("users", toListJson(Collections.singletonList(list)));
                }
            }
        });
        channel.invokeMethod("onReceiveGroupAdminApproval", json);
    }
    ...
    public void onEventMainThread(GroupApprovalRefuseEvent event) {
        ...
        channel.invokeMethod("onReceiveGroupAdminReject", json);
